        ...
        <div className="fixed inset-y-0 right-0 z-50 w-full max-w-md bg-black/50 backdrop-blur-lg border-l border-white/10 animate-pop-in">
            <div className="flex flex-col h-full">
                ...
                <div className="flex-1 p-6 overflow-y-auto scrollbar-hide space-y-6">
                    <div>
                        <p className="text-sm font-medium text-zinc-400 mb-1">Question:</p>
                        <p className="text-white">{question.question.replace('___', `[${question.correct_answer}]`)}</p>
                    </div>
                    <div>
                        <p className="text-sm font-medium text-zinc-400 mb-1">Your Answer:</p>
                        <p className="text-red-400">{userAnswer}</p>
                    </div>
                    <div>
                        <p className="text-sm font-medium text-zinc-400 mb-1">Explanation:</p>
                        {explanation ? (
                            <p className="text-zinc-200 whitespace-pre-wrap">{explanation}</p>
                        ) : (
            ...
            case 'topic':
                return (
                    <div className="text-center animate-pop-in">
                        ...
                        <p className="text-zinc-400 mb-8">What topic would you like to be quizzed on?</p>
                        <form onSubmit={handleTopicSubmit} className="max-w-lg mx-auto">
                            <input
                                type="text"
                                value={topic}
                            ...
                            />
                            <button type="submit" className="mt-6 bg-indigo-600 hover:bg-indigo-500 text-white px-8 py-3 rounded-full font-medium transition-all duration-300 shadow-[0_0_20px_rgba(129,140,248,0.4)] hover:shadow-[0_0_30px_rgba(129,140,248,0.6)] interactive-lift">
                                Generate Quiz
                            </button>
                        </form>
                    </div>
                ...
                return (
                    <div className="w-full max-w-3xl animate-pop-in">
                        <div className="text-center mb-8">
                            <p className="text-indigo-400 font-semibold">{quizData.topic}</p>
                            <h2 className="text-2xl font-bold">Question {currentQuestionIndex + 1} of {quizData.questions.length}</h2>
                        </div>
                        <div data-liquid-glass className="liquid-glass p-8 rounded-2xl">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    {question.options?.map(option => (
                                        <button
                                            ...
                                            className={`p-4 rounded-xl border text-left transition-all duration-300 disabled:cursor-not-allowed ${selectedAnswer === option ? 'bg-indigo-500/30 border-indigo-500/60' : 'bg-white/5 border-white/10 hover:bg-white/10'} ${isAnswered && option === feedback?.correct_answer ? '!bg-green-500/30 !border-green-500/60' : ''} ${isAnswered && selectedAnswer === option && !feedback?.correct ? '!bg-red-500/30 !border-red-500/60' : ''}`}
                                        >
                                            {option}
                                        </button>
                                    ))}
                            ...
                            {(question.type === 'short-answer' || question.type === 'fill-in-the-blank') && (
                                <textarea
                                    ...
                                    disabled={isAnswered}
                                    rows={3}
                                    className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white placeholder-zinc-500 focus:outline-none focus:border-indigo-500/50 focus:ring-2 focus:ring-indigo-500/30 transition-all duration-300"
                            ...
                            <div className="mt-6 text-center">
                                {isAnswered ? (
                                    <div className="space-y-4">
                                        {feedback?.correct ? (
                                            <p className="text-green-400 font-bold text-lg">Correct! (+{feedback.score} points)</p>
                                        ) : (
                                            <div>
                                                <p className="text-red-400 font-bold text-lg">Incorrect (+{feedback?.score} points)</p>
                                                {question.type !== 'multiple-choice' && <p className="text-zinc-400 text-sm">Ideal Answer: {feedback?.correct_answer}</p>}
                                                <button onClick={handleOpenExplanation} className="mt-2 text-sm text-indigo-400 hover:underline">Why did I get it wrong?</button>
                                            </div>
                                        )}
                                        <button onClick={handleNextQuestion} className="bg-indigo-600 hover:bg-indigo-500 text-white px-8 py-3 rounded-full font-medium transition-all duration-300">
                                            {currentQuestionIndex === quizData.questions.length - 1 ? 'Finish Quiz' : 'Next Question'}
                                        </button>
                                    </div>
                                ) : (
                                    <button onClick={handleAnswerSubmit} disabled={!selectedAnswer.trim() || isEvaluating} className="bg-white hover:bg-zinc-200 text-black px-8 py-3 rounded-full font-medium transition-all duration-300 disabled:bg-zinc-700 disabled:text-zinc-400 disabled:cursor-not-allowed">
            ...
            case 'finished':
                ...
                return (
                    <div className="w-full max-w-3xl text-center animate-pop-in">
                        ...
                        <p className="text-zinc-400 mb-8">You finished the quiz on {quizData?.topic}.</p>
                        ...
                        <div className="space-y-4 text-left">
                            ...
                            {quizData?.questions.map((q, i) => {
                                const userAnswer = userAnswers.find(a => a.questionIndex === i);
                                return (
                                    <div key={i} data-liquid-glass className={`liquid-glass p-4 rounded-xl border ${userAnswer?.isCorrect ? 'border-green-500/30' : 'border-red-500/30'}`}>
                                        <p className="font-semibold">{i + 1}. {q.question.replace('___', `[${q.correct_answer}]`)}</p>
                                        <p className={`text-sm ${userAnswer?.isCorrect ? 'text-green-400' : 'text-red-400'}`}>Your answer: {userAnswer?.answer}</p>
                                        {!userAnswer?.isCorrect && q.type !== 'multiple-choice' && <p className="text-xs text-zinc-400 mt-1">Correct answer: {q.correct_answer}</p>}
                                    </div>
                                );
                            })}
                        </div>
                        <button onClick={() => setQuizState('topic')} className="mt-8 bg-white/10 hover:bg-white/20 text-white px-8 py-3 rounded-full font-medium transition-all duration-300">Take another quiz</button>
                    </div>
                );
        }
    };
    ...
    return (
        <>
            ...
            <div className="fixed inset-0 z-10 flex flex-col items-center justify-center p-4 text-white overflow-y-auto scrollbar-hide">
                <button onClick={() => navigate('/chat')} className="absolute top-6 left-6 p-2 rounded-full text-zinc-300 hover:text-white bg-black/20 hover:bg-black/40 backdrop-blur-md border border-white/10 transition-colors">
