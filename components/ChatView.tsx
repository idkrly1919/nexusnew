...
const ChatView: React.FC = () => {
    ...
    const processSubmission = async (userText: string) => {
        ...
        try {
            ...
            for await (const update of stream) {
                if (isFirstChunk) {
                    // Remove thinking indicator once we have real data or thinking started streaming
                    setMessages(prev => prev.filter(m => m.role !== 'typing'));
                    // Add the empty AI message to fill
            ...
            setMessages(prev => prev.filter(m => m.role !== 'typing')); // Ensure thinking is gone
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const parseMarkdown = (text: string) => {
        ...
        // Replace images with HTML + Download Button
        ...
        <div id="chat-view" className="fixed inset-0 z-50 flex flex-col bg-[#18181b] text-zinc-100 font-sans overflow-hidden">
            ...
            <main className="flex-1 relative flex flex-col overflow-hidden">
                ...
                <div className="flex-1 overflow-y-auto relative z-10 scrollbar-hide">
                    {messages.length === 0 ? (
                        ...
                        <div className="max-w-3xl mx-auto px-4 py-8 space-y-8">
                                        ...
                                        <div className="flex-1 space-y-2 min-w-0">
                                            ...
                                            {!isLoading && (
                                                <div className="flex items-center gap-2 mt-2">
                                                     ...
                                                     <button onClick={() => navigator.clipboard.writeText(msg.text)} className="p-1.5 text-zinc-500 hover:text-zinc-300 hover:bg-white/5 rounded-md transition-colors" title="Copy">
                                            ...
                                            {msg.groundingChunks && msg.groundingChunks.length > 0 && (
                                                <div className="mt-3 flex gap-2 flex-wrap">
                                                    {msg.groundingChunks.slice(0, 3).map((chunk, i) => (
                                                        <a key={i} href={chunk.web.uri} target="_blank" rel="noopener noreferrer" 
                                                           className="flex items-center gap-1.5 bg-zinc-800/50 hover:bg-zinc-800 border border-white/5 px-3 py-1.5 rounded-full text-xs text-zinc-400 hover:text-zinc-200 transition-colors max-w-[200px] truncate">
                                                            <svg className="shrink-0 w-3 h-3" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10"/><line x1="2" y1="12" x2="22" y2="12"/><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1 4-10 15.3 15.3 0 0 1 4-10z"/></svg>
                            ...
                            <div className="h-48" ref={messagesEndRef} />
                        </div>
                    )}
                </div>
                ...
                <div className="absolute bottom-6 left-0 right-0 px-4 z-30">
                    <div className="max-w-3xl mx-auto bg-[#1f1f22] border border-[#303033] rounded-[28px] shadow-2xl relative overflow-hidden transition-colors focus-within:border-zinc-600">
                        
                        {/* Input Container Flex Column */}
                        <div className="flex flex-col">
                            
                            {/* File Attachment Pill (Static Flow) */}
                            {attachedFile && (
                                    ...
                                    <div className="flex items-center justify-between bg-[#2a2a2d] border border-white/10 rounded-lg px-3 py-2 animate-in fade-in zoom-in-95 duration-200">
                                        <div className="flex items-center gap-2 overflow-hidden">
                                            ...
                                            <div className="flex flex-col min-w-0">
                                                ...
                                                <span className="text-[10px] text-zinc-500 truncate">{attachedFile.type || 'Unknown Type'}</span>
                                            </div>
                                        </div>
                                        <button onClick={removeFile} className="text-zinc-500 hover:text-zinc-300 p-1 rounded-full hover:bg-white/5">
                            ...
                            {/* Text Input */}
                            <form onSubmit={handleChatSubmit} className="flex flex-col relative z-10">
                                <input 
                                    type="file" 
                                    ref={fileInputRef} 
                                ...
                                <textarea
                                    ...
                                    rows={1}
                                    placeholder={attachedFile ? "Add a message..." : "Message Nexus..."}
                                    className={`w-full bg-transparent text-zinc-200 placeholder-zinc-500 px-5 py-4 focus:outline-none resize-none max-h-48`}
                                ...
                                <div className="flex items-center justify-between px-3 pb-3 pt-1">
                                    ...
                                    {/* Left Tools */}
                                    <div className="flex items-center gap-2">
                                        <button type="button" className="flex items-center gap-1.5 bg-[#2a2a2d] hover:bg-[#333336] text-zinc-300 hover:text-white text-xs font-medium px-3 py-1.5 rounded-full border border-white/5 transition-colors">
                                            <svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"/><rect x="7" y="7" width="3" height="9"/><rect x="14" y="7" width="3" height="5"/></svg>
                                            <span>Nexus K4 Pro</span>
                                        ...
                                        <button type="button" onClick={triggerFileSelect} className="p-2 text-zinc-400 hover:text-zinc-100 hover:bg-white/5 rounded-full transition-colors" title="Attach File">
                                            <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="m21.44 11.05-9.19 9.19a6 6 0 0 1-8.49-8.49l8.57-8.57A4 4 0 1 1 18 8.84l-8.59 8.57a2 2 0 0 1-2.83-2.83l8.49-8.48"/></svg>
                                        </button>
                                        
                                        <button 
                                            type="button" 
                                            onClick={() => setIsReasoningEnabled(!isReasoningEnabled)}
                                            className={`p-2 rounded-full transition-colors ${isReasoningEnabled ? 'text-white bg-white/10' : 'text-zinc-400 hover:text-zinc-100 hover:bg-white/5'}`} 
                                            title="Reasoning"
                                        >
                                            {/* Brain Icon */}
                                    ...
                                    <div className="flex items-center gap-2">
                                        <button 
                                            ...
                                            className={`p-2 rounded-full transition-colors ${isListening ? 'text-white bg-white/10 animate-pulse' : 'text-zinc-400 hover:text-zinc-100 hover:bg-white/5'}`}
                                            title="Voice Input"
                                        >
                                        ...
                                        <button 
                                            ...
                                            className={`p-2 rounded-full transition-all ${inputValue.trim() || attachedFile ? 'bg-zinc-200 text-black hover:bg-white shadow-lg shadow-white/10' : 'bg-[#303033] text-zinc-600 cursor-not-allowed'}`}
                                        >
                                            <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/></svg>
                                        </button>
                                    </div>
                                </div>
                            </form>
                    ...
                    <div className="text-center mt-4">
                         <p className="text-[10px] text-zinc-600 font-medium tracking-wider">NEXUS IS AN EXPERIMENTAL AI SYSTEM</p>
                    </div>
                </div>
